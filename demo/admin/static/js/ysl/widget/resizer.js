(function(Y){
	...
	var Resizer = function(tag, option){
		...
		this.isAbs = this.tag.getStyle('position') == 'absolute';
		var tagIdx = this.isAbs ? this.tag.getStyle('zIndex') : 0;

		if(this.isAbs && tagIdx){
			this.dom.setStyle('z-index', tagIdx+1);
		}

		this.dom = Y.dom.one('body').create('div');
	...
	**/
	...
	Resizer.prototype.getDom = function(){
		return this.dom;
	};
