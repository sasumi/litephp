...
class _OAuthToken { 
    ...
    function to_string() { 
        return "oauth_token=" . 
            _OAuthUtil::urlencode_rfc3986($this->key) . 
            "&oauth_token_secret=" . 
            _OAuthUtil::urlencode_rfc3986($this->secret); 
    } 
 ...
 * @ignore 
 */ 
class __OAuthSignatureMethod_HMAC_SHA1 extends _OAuthSignatureMethod { 
    function get_name() { 
        return "HMAC-SHA1"; 
    } 
    ...
    public function build_signature($request, $consumer, $token) { 
        ...
        //print_r( $key_parts );
		$key_parts = _OAuthUtil::urlencode_rfc3986($key_parts); 
 ...
 * @ignore 
 */ 
class __OAuthSignatureMethod_PLAINTEXT extends _OAuthSignatureMethod { 
    ...
    public function build_signature($request, $consumer, $token) { 
        $sig = array( 
            _OAuthUtil::urlencode_rfc3986($consumer->secret) 
        ); 

        if ($token) { 
            array_push($sig, _OAuthUtil::urlencode_rfc3986($token->secret)); 
        } else { 
            array_push($sig, ''); 
        } 
        ...
        return _OAuthUtil::urlencode_rfc3986($raw); 
    } 
} 
 ...
 * @ignore 
 */ 
class __OAuthSignatureMethod_RSA_SHA1 extends _OAuthSignatureMethod { 
    public function get_name() { 
        return "RSA-SHA1"; 
    } 
...
class _OAuthRequest { 
    ...
    public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) { 
        ...
        if (!$parameters) { 
            ...
            $request_headers = _OAuthUtil::get_headers(); 

            // Parse the query-string to find GET parameters 
            $parameters = _OAuthUtil::parse_parameters($_SERVER['QUERY_STRING']); 
            ...
            if ($http_method == "POST" 
                ...
                $post_data = _OAuthUtil::parse_parameters( 
            ...
            if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") { 
                $header_parameters = _OAuthUtil::split_header( 
        ...
        return new _OAuthRequest($http_method, $http_url, $parameters); 
    } 
    ...
    public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL) { 
        @$parameters or $parameters = array(); 
        $defaults = array("oauth_version" => _OAuthRequest::$version, 
            "oauth_nonce" => _OAuthRequest::generate_nonce(), 
            "oauth_timestamp" => _OAuthRequest::generate_timestamp(), 
    ...
    public function get_signable_parameters() { 
        ...
        return _OAuthUtil::build_http_query($params); 
    } 
    ...
    public function get_signature_base_string() { 
        ...
        $parts = _OAuthUtil::urlencode_rfc3986($parts); 
    ...
    //echo "multi=" . $multi . '`';
    if( $multi )
    	return _OAuthUtil::build_http_query_multi($this->parameters); 
    else 
        return _OAuthUtil::build_http_query($this->parameters); 
    } 
    ...
    public function to_header() { 
        ...
        foreach ($this->parameters as $k => $v) { 
            ...
            $out .= ',' . 
                _OAuthUtil::urlencode_rfc3986($k) . 
                '="' . 
                _OAuthUtil::urlencode_rfc3986($v) . 
                '"'; 
        } 
        return $out; 
    } 
...
class _OAuthUtil { 
    ...
    public static function urlencode_rfc3986($input) { 
        if (is_array($input)) { 
            return array_map(array('_OAuthUtil', 'urlencode_rfc3986'), $input); 
    ...
    public static function split_header($header, $only_allow_oauth_parameters = true) { 
        ...
        while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) { 
            ...
            if (preg_match('/^oauth_/', $header_name) || !$only_allow_oauth_parameters) { 
                $params[$header_name] = _OAuthUtil::urldecode_rfc3986($header_content); 
            } 
    ...
    public static function parse_parameters( $input ) { 
        ...
        foreach ($pairs as $pair) { 
            ...
            $parameter = _OAuthUtil::urldecode_rfc3986($split[0]); 
            $value = isset($split[1]) ? _OAuthUtil::urldecode_rfc3986($split[1]) : ''; 
    ...
    public static function build_http_query_multi($params) { 
        ...
        //$keys = _OAuthUtil::urlencode_rfc3986(array_keys($params)); 
        //$values = _OAuthUtil::urlencode_rfc3986(array_values($params)); 
...
class WeiboOAuth { 
    ...
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) { 
        $this->sha1_method = new __OAuthSignatureMethod_HMAC_SHA1(); 
        $this->consumer = new _OAuthConsumer($consumer_key, $consumer_secret); 
        if (!empty($oauth_token) && !empty($oauth_token_secret)) { 
            $this->token = new _OAuthConsumer($oauth_token, $oauth_token_secret); 
        } else { 
            $this->token = NULL; 
        } 
    } 
    ...
    function getRequestToken($oauth_callback = NULL) { 
        ...
        $token = _OAuthUtil::parse_parameters($request); 
        $this->token = new _OAuthConsumer($token['oauth_token'], $token['oauth_token_secret']); 
        return $token; 
    } 
    ...
    function oAuthRequest($url, $method, $parameters , $multi = false) { 
        ...
        // echo $url ; 
        $request = _OAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters); 
...
*/
        ...
        $header_array2=array(); 
        if( $multi ) 
        	$header_array2 = array("Content-Type: multipart/form-data; boundary=" . _OAuthUtil::$boundary , "Expect: ");
