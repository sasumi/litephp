...
class Resume {
	public static function getResume($resume_id){
		...
		if($resume_id){
			$current_resume = DBM::instance('resume')->find("id=? AND user_id=?", $resume_id, $user_id)->one();
			if($current_resume->id){
				$template_item_config = ResumeMod::init()->getAllMods();

				$datas = DBM::instance('resume_data')->find('resume_id=?', $resume_id)->order('order DESC')->all;
				$tmp = array(
					'cover' => array(
						'item_type' => 'cover',
						'item_title' => '封面',
