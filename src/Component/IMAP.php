...
/**
 * Features
 * Connect to mailbox by POP3/IMAP/NNTP, using imap_open()
 * Get mailbox status, usinge imap_check()
 * Receive emails (+attachments, +html body images)
 * Search emails by custom criteria, using imap_search()
 * Change email status, using imap_setflag_full()
 * Delete email, using imap_delete()
 * Requirements
...
class IMAP{
	...
	/**
	 * To connect to an POP3 server
	 * @param $server
	 * @param int $port
	 * @param bool $ssl
	 */
	public function switchConnectPOP3($server, $port = 995, $ssl = true){
		$port = $port ?: ($ssl ? '995' : '110');
		$this->imapHost = '{'.$server.':'.$port.'/pop3'.($ssl ? "/ssl" : "").'}';
		$this->setImapPath($this->imapHost);
	}
	 ...
	 * @throws
	 */
	public function switchMailbox($boxDir = 'INBOX'){
		$boxPath = function_exists('mb_convert_encoding') ? mb_convert_encoding($boxDir, "UTF7-IMAP", "UTF-8") : imap_utf7_encode($boxDir);
		$this->imapPath = $this->imapHost.$boxPath;
	...
	public function searchMailbox($criteria = 'ALL', $options = SE_UID){
		...
		if(strpos($this->imapHost, 'imap-mail.outlook.com') !== false){
			$serverEncoding = 'US-ASCII';
		}
		$mailsIds = imap_search($this->getImapStream(), $criteria, $options, $serverEncoding);
		if($mailsIds === false){
			throw new Exception('Not understand the search criteria: '.$criteria);
		}
	 ...
	 * @return array in the case of call failure
	 */
	...
	public function getMail($mailId, $markAsSeen = true){
		...
		$timestamp = time();
		if(isset($head->date)){
			$date = preg_replace('/\(.*?\)/', '', $head->date);
			if(preg_match("/[^\\d]*([\\d]{10})[^\\d]*/i", $date, $match)){
				$timestamp = $match[1];
			} else{
		...
		$mail->fromAddress = strtolower($this->decodeMimeStr($head->from[0]->mailbox, $this->serverEncoding).'@'.$head->from[0]->host);
		if(isset($head->to)){
			$toStrings = array();
			foreach($head->to as $to){
				if(!empty($to->mailbox) && !empty($to->host)){
					$toEmail = strtolower($this->decodeMimeStr($to->mailbox, $this->serverEncoding).'@'.$to->host);
		...
		if(isset($head->cc)){
			foreach($head->cc as $cc){
				$mail->cc[strtolower($this->decodeMimeStr($cc->mailbox, $this->serverEncoding).'@'.$cc->host)] = isset($cc->personal) ? $this->decodeMimeStr($cc->personal, $this->serverEncoding) : null;
			}
		}
		if(isset($head->bcc)){
			foreach($head->bcc as $bcc){
				$mail->bcc[strtolower($this->decodeMimeStr($bcc->mailbox, $this->serverEncoding).'@'.$bcc->host)] = isset($bcc->personal) ? $this->decodeMimeStr($bcc->personal, $this->serverEncoding) : null;
			}
		}
		if(isset($head->reply_to)){
			foreach($head->reply_to as $replyTo){
				$mail->replyTo[strtolower($this->decodeMimeStr($replyTo->mailbox, $this->serverEncoding).'@'.$replyTo->host)] = isset($replyTo->personal) ? $this->decodeMimeStr($replyTo->personal, $this->serverEncoding) : null;
			}
		}
	...
	/**
	 * 获取邮件正文/附件
	 * @return \Lite\Component\Mail
	 * @param bool $markAsSeen
	 * @param \Lite\Component\Mail $mail
	 */
	 ...
	 * 下载附件
	 * @param $targetMailId
	 * @param $targetAttachmentName
	 * @return array|\Lite\Component\MailAttachment
	 */
	public function getAttachmentData($targetMailId, $targetAttachmentName){
		$attachment = [];
		$mail = new Mail();
		$mail->id = $targetMailId;
		$markAsSeen = false;
		$mailStructure = imap_fetchstructure($this->getImapStream(), $targetMailId, FT_UID);
		if($mailStructure->parts){
			foreach($mailStructure->parts as $partNum => $partStructure){
				$attachment = $this->getAttachmentMailPart($mail, $partStructure, $partNum+1, $markAsSeen, $targetAttachmentName);
				if($attachment){
					break;
				}
			}
	...
	protected function getAttachmentMailPart(Mail $mail, $partStructure, $partNum, $markAsSeen, $targetAttachmentName){
		$attachment = [];
		$options = FT_UID;
		if(!$markAsSeen){
			$options |= FT_PEEK;
		...
		if($attachmentId){
			...
			if($fileName == $targetAttachmentName){
				$attachment = new MailAttachment();
				$attachment->id = $attachmentId;
				$attachment->name = $fileName;
				$attachment->size = $partStructure->dparameters[1]->value;
				$attachment->disposition = (isset($partStructure->disposition) ? $partStructure->disposition : null);
				...
				$attachment->dataStream = $data;
			}
		}
		if(!empty($partStructure->parts)){
			foreach($partStructure->parts as $subPartNum => $subPartStructure){
				if($partStructure->type == 2 && $partStructure->subtype == 'RFC822'){
					$attachment = $this->getAttachmentMailPart($mail, $subPartStructure, $partNum, $markAsSeen, $targetAttachmentName);
				} else{
					$attachment = $this->getAttachmentMailPart($mail, $subPartStructure, $partNum.'.'.($subPartNum+1), $markAsSeen, $targetAttachmentName);
				}
			}
		}
		return $attachment;
