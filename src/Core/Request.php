...
abstract class Request {
	 ...
	 * @throws Exception
	 */
	private static function checkRsp($curl, $url){
		$curl_errno = curl_errno($curl);
		if($curl_errno > 0){
			throw new Exception($curl_errno);
		}
		$http_code = curl_getinfo($curl, CURLINFO_HTTP_CODE);
		if($curl_errno > 0 || !preg_match('/^2\D{2}$/', $http_code)){
			$msg = array('REQUEST URL:'.$url);
			if($curl_errno){
				$msg[] = 'CURL ERROR:'.$curl_errno.' - '.curl_error($curl);
			}
			if(!preg_match('/^2\D{2}$/', $http_code)){
				$msg[] .= 'HTTP STATUS CODE:'.$http_code.' - '.self::$HTTP_STATUS[$http_code];
			}
